...
with open('./dataset.json', 'w') as jf:
...
for datapoint in outputData.keys():
    ...
    for run in outputData[datapoint].keys():
        ...
        for pos in outputData[datapoint][run].keys():
            ...
            for d in db[run][pos]:
                ...
                if run != 'base':
                    ...
                    baseValue = smallNumberCheck(float(basedict[d['beta']][d['Mach']][d['AoA']]))
                    v = smallNumberCheck(float("%.5f" % (float(bv) - baseValue)))
                    try:
        ...
        if run != 'base':
            ...
            degList.sort()
            print(degList_unsorted, degList)
            for deg in degList:
                if deg % 1 == 0:
                    deg = int(deg)
